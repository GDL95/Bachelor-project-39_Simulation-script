...
MySimulationCell<- function(Design = Design, RowOfDesign = 1, K=1){
  ...
  for (k in 1:K){
    ...
    MyAnalysisResult2 <- Method_old(SimDat)$p.value
    #Combine relevant results of the analysis by the two methods in a vector (optional)
    MyAnalysisResult <- c(MyAnalysisResult1, MyAnalysisResult2)
    #Evaluate the analysis results of Method_new (Result1) and Mehtod_old (Result2)
    
    MyResult1 <- MyEvaluation(MyAnalysisResult1, es)
    MyResult2 <- MyEvaluation(MyAnalysisResult2, es)
...
Row <- 1
MyResult <- MySimulationCell(Design = Design, RowOfDesign = Row, K = 1 )
